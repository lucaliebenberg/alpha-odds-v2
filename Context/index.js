...
// TODO: DEPLOY PREDICTION_MARKET TO LOCAL NODE AND TEST
import { PREDICTION_MARKET_ADDRESS, PREDICTION_MARKET_ABI } from "./constants";
import { connectingPredictionMarketContract } from "../Utils/index";
...
export const StateContextProvider = ({ children }) => {
  ...
  const createPredictionMarketContract = async (description, endTimestamp, price) => {
    try {
      const contract = await connectingPredictionMarketContract();
      const transaction = await contract.createContract(description, endTimestamp, price);
      await transaction.wait();
      console.log("Prediction market contract created:", transaction);
    } catch (error) {
      console.error("Error creating prediction market contract:", error);
    }
  };

  const buyShares = async (contractIndex, outcome, amount) => {
    try {
      const contract = await connectingPredictionMarketContract();
      const transaction = await contract.buyShares(contractIndex, outcome, { value: amount });
      await transaction.wait();
      console.log("Shares bought:", transaction);
    } catch (error) {
