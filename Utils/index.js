...
// PREDICTION MARKET UTILS
const fetchPredictionMarketContract = (signerOrProvider) =>
  new ethers.Contract(PREDICTION_MARKET_ADDRESS, PREDICTION_MARKET_ABI, signerOrProvider);

export const connectingPredictionMarketContract = async () => {
  try {
    const web3modal = new Web3Modal();
    const connection = await web3modal.connect();
    const provider = new ethers.providers.Web3Provider(connection);
    const signer = provider.getSigner();
    const contract = fetchPredictionMarketContract(signer);
    return contract;
  } catch (error) {
    console.log(error);
  }
